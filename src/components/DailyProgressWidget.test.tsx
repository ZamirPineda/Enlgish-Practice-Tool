...
describe("DailyProgressWidget", () => {
  ...
  it("shows session summary and allows claiming the daily session reward once", () => {
    ...
    const rewardButtonBefore = screen.getByRole("button", {
      name: "Play 2 sessions with 70% or more accuracy to unlock daily reward",
    });
    ...
    expect(screen.getByRole("button", { name: "Daily reward already claimed" })).toBeDisabled();
  ...
  it("claims weekly consistency reward when enough active days are reached", () => {
    ...
    const weeklyClaimButton = screen.getAllByRole("button", {
      name: "Claim +30 XP for completing Starter tier",
    })[0];
    ...
    expect(screen.getByRole("button", { name: "Starter tier reward already claimed" })).toBeDisabled();
  });
});

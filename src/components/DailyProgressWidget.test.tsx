...
describe("DailyProgressWidget", () => {
  ...
  it("shows session summary and allows claiming the daily session reward once", () => {
    ...
    const rewardButtonBefore = screen.getByRole("button", {
      name: /Play 2 sessions with 70%\+ accuracy to unlock daily reward|Claim daily session reward of 40 XP/i,
    });
    ...
    expect(screen.getByRole("button", { name: /Daily session reward already claimed/i })).toBeDisabled();
  ...
  it("claims weekly consistency reward when enough active days are reached", () => {
    ...
    expect(screen.getByRole("button", { name: /Weekly .* reward claimed/ })).toBeDisabled();
  });
});

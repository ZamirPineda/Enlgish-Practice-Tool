...
const MathFlashCard: React.FC<MathFlashCardProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Don't hijack standard buttons or inputs
      const activeTagName = document.activeElement?.tagName;

      const isStandardInteractive = ["BUTTON", "INPUT", "TEXTAREA", "A"].includes(activeTagName || "");

      // If a standard interactive element like a <button> is focused, we should
      // let it handle Space and Enter on its own, so we don't flip the card.
      // But we STILL want ArrowRight/ArrowLeft/Escape to work globally.
      if (isStandardInteractive && (e.key === "Enter" || e.key === " ")) {
        return;
      }
      ...
      if (e.key === "ArrowRight") {
        ...
        e.preventDefault();
        onExit();
      } else if (!isStandardInteractive && (e.key === " " || e.key === "Enter")) {
        e.preventDefault();
        handleFlip();
      }
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 h-full max-w-2xl mx-auto">
      ...
      <div
        ...
        className="w-full relative min-h-[400px] md:min-h-[500px] cursor-pointer perspective-1000 group focus:outline-none focus:ring-2 focus:ring-primary rounded-2xl"
        style={{ perspective: "1000px" }}
        onClick={handleFlip}
      >
        <span className="sr-only" lang="es">
          Tarjeta de matemáticas. Presiona espacio o enter para girar. Usa las

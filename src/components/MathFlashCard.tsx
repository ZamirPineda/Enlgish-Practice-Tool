  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        ...
        const isStandardInteractive = tagName === "BUTTON" || tagName === "A";
        const isOurCard = cardRef.current?.contains(target);

        if (isStandardInteractive) return;
        if (target.getAttribute("role") === "button" && !isOurCard) return;

...
const MathFlashCard: React.FC<MathFlashCardProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Do not hijack typing in inputs or native buttons/links
      if (
        tagName === "INPUT" ||
        tagName === "TEXTAREA" ||
        tagName === "SELECT" ||
      ...
      if (e.key === "ArrowRight") {
        ...
        // Only prevent default and flip if we aren't focused on another button
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 h-full max-w-2xl mx-auto">
      ...
      {/* Card Container */}
      <div
        className="w-full relative min-h-[400px] md:min-h-[500px] cursor-pointer perspective-1000 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-2xl"
      ...
      >
        <span className="sr-only" lang="es">
          Tarjeta de memoria matemática. Pulsa Enter o Espacio para girar.
        </span>
        <div

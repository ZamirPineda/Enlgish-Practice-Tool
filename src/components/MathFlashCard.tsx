...
const MathFlashCard: React.FC<MathFlashCardProps> = ({
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      // If we are focused on a generic interactive element, we should generally not hijack space/enter
      // EXCEPT when we are focused on the flashcard itself (which we gave role="button" and id="flashcard-container")
      // because we WANT arrows and escape to work while focused on the card.
      const isCardFocused = target?.id === "flashcard-container";
    ...
    <div className="flex flex-col items-center justify-center p-6 h-full max-w-2xl mx-auto">
      ...
      >
        <span className="sr-only">
          Presiona Enter o Espacio para girar la tarjeta. Usa las flechas para
          navegar. Esc para salir.
        </span>
        <div

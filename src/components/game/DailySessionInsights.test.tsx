...
describe("DailySessionInsights", () => {
  ...
  it("shows locked reward by default", () => {
    ...
    expect(screen.getByRole("button", { name: "Claim +40 XP for daily session" })).toBeDisabled();
  });
  ...
  it("claims reward once when eligible", () => {
    ...
    const claimButton = screen.getByRole("button", { name: "Claim +40 XP for daily session" });
    ...
    expect(screen.getByRole("button", { name: "Daily session reward already claimed" })).toBeDisabled();

        ...
        <button
          ...
          aria-label={
            sessionSummary.rewardClaimed
              ? "Reward Claimed (Daily session reward already claimed)"
              : sessionSummary.rewardEligible
                ? `Claim +${sessionSummary.rewardXp} XP (Daily session reward ready)`
                : `Claim +${sessionSummary.rewardXp} XP (Locked: play 2 sessions with 70%+ accuracy to unlock)`
          }
        >
          ...
          {weeklyConsistency.tiers.map((tier) => (
            ...
            >
              ...
              <button
                ...
                aria-label={
                  tier.claimed
                    ? `Claimed (Weekly reward ${tier.title} already claimed)`
                    : tier.eligible
                      ? `Claim Reward (+${tier.rewardXp} XP for weekly consistency: ${tier.title})`
                      : `Locked (Keep weekly streak to claim ${tier.title})`
                }
              >
                {tier.claimed
                  ? "Claimed"

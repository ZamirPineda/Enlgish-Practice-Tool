...
export function calculateSrsData(
  ...
  if (correct) {
    ...
    const quality = 5;
    const efactor = Math.max(1.3, item.efactor + (0.1 - (5 - quality) * (0.08 + (5 - quality) * 0.02)));
 ...
 */
export function createNewSrsItem(word: string, definition: string): SrsVocabularyItem {
 ...
 */
export function getDueReviewItems(deck: Record<string, SrsVocabularyItem>): SrsVocabularyItem[] {
    if (!deck) return [];
    const today = new Date().toISOString().split('T')[0];
 ...
 */
export function getDueReviewWords(deck: Record<string, SrsVocabularyItem>): string[] {
    return getDueReviewItems(deck).map(item => item.word);
}

export const getSrsLocalStorageKey = (level: string) => `srs-vocabulary-deck-${level}`

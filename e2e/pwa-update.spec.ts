...
test.describe("PWA Auto Update Flow", () => {
  test("Shows update banner when in active session (mocked SW update)", async ({
    ...
    await page.waitForFunction(() => document.body.textContent?.includes("Nueva versión disponible") || document.querySelector(".pwa-update-banner")); await expect(updateBanner).toBeVisible();
    ...
    await page.waitForTimeout(1000); await page.evaluate(() => true); expect(didReload).toBe(true);
  });

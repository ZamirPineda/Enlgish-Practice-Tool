...
test.describe("PWA Auto Update Flow", () => {
  test("Shows update banner when in active session (mocked SW update)", async ({
    ...
    // Alternatively, just verify that the function executes window.location.reload()
    // It's possible framenavigated isn't caught reliably in this setup, so let's fallback:

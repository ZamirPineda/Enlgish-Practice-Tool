  ...
  const renderTopicTable = (
    ...
    <>
      <div className="text-center mb-6">
        <p className="text-slate-400 text-xs md:text-sm">{topic.description}</p>
      ...
      {!isStudyMode
        ? // NORMAL VIEW: Tables
          topic.sections.map((section, idx) => (
            <div
              key={idx}
              className="bg-slate-800/50 rounded-xl border border-slate-700 overflow-hidden shadow-sm mb-8"
            >
              {/* ... Existing Table Code ... */}
                  ...
                  <thead className="bg-slate-900/50 text-slate-300 font-semibold">
                    <tr>
                      {section.headers.map((h, i) => (
                        <th
                          key={i}
                          className="p-3 border-b border-slate-700 min-w-[120px] whitespace-nowrap first:min-w-[100px]"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {section.rows.map((row, rIdx) => (
                      ...
                      >
                        {row.map((cell, cIdx) => (
                          <td
                            key={cIdx}
                            className={`p-3 text-slate-200 ${cIdx === 0 ? "font-bold " + colorClass.replace("400", "200") : "font-mono"}`}
                          >
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-900">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8 pb-20 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        ...
        {activeTab === "algebra" &&
          renderTopicTable(algebraTopic, "text-violet-400", "text-violet-200")}

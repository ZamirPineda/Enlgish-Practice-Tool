...
| **Cociente**      | $\frac{u}{v}$      | $\frac{u'v - uv'}{v^2}$  | _(No hay directa)_           | $\frac{x}{e^x} \to \frac{e^x - xe^x}{e^{2x}}$ |
| **Cadena**        | $f(g(x))$          | $f'(g(x)) \cdot g'(x)$   | _(Sustitución)_              | $\sin(x^2) \to \cos(x^2)\cdot 2x$             |

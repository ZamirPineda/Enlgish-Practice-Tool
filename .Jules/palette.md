...
## 2026-03-08 - [Keyboard Focus for Visually Hidden Inputs]

**Learning:** Components using `sr-only` inputs (like ToggleSwitch) often lose visible focus indicators because the input is hidden.
**Action:** Use the `peer` class on the hidden input and `peer-focus-visible:*` utility classes on the visible sibling element to restore keyboard focus rings.

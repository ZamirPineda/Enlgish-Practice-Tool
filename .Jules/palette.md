...
\n## 2026-07-09 - Keyboard Accessibility for Custom Flashcard Components\n**Learning:** Custom interactive elements like flashcards implemented as `div` elements require comprehensive keyboard support (Space/Enter to activate, Arrow keys for navigation) and proper ARIA roles (`role="button"`, `tabIndex={0}`) to be usable by screen reader and keyboard-only users. Global keydown listeners for these shortcuts must check `document.activeElement.tagName` to avoid hijacking native interactions on other inputs.\n**Action:** When creating or modifying custom interactive 'card' components, always ensure they are fully keyboard-navigable and semantically identified as buttons if they perform actions when clicked.

...
## 2026-03-24 - [Math Flashcard Accessibility and Global Keyboard Navigation]
**Learning:** Custom flashcards implemented with `div` often miss critical accessibility attributes (`role="button"`, `tabIndex={0}`, focus styling) and local keydown handlers for Space and Enter, breaking standard interaction patterns for screen reader and keyboard users. Additionally, when implementing global keyboard navigation (like Arrow keys for next/prev), failing to ignore native text inputs can mistakenly hijack default behavior.
**Action:** When creating or updating custom interactive cards (like MathFlashCard), ensure they are fully accessible by adding explicit roles, focus styles, and providing visually hidden (`sr-only`) instructions to guide screen-readers without masking visible child content. For global keydown handlers, strictly wrap dependencies in `useCallback` to prevent stale state, and explicitly filter out interactive HTML tags (`INPUT`, `TEXTAREA`, etc.) before calling `preventDefault()`.

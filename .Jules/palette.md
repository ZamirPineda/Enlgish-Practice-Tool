...
**Learning:** Buttons that toggle the visibility of content (expand/collapse) often lack semantic state and connections to their controlled panels, making the interface opaque to screen reader users. Also, complex emojis inside toggle text can cause verbose screen reader announcements.
**Action:** Always add `aria-expanded` reflecting the toggle state, `aria-controls` pointing to a unique ID (generated by `useId()` for repeated components) on the conditionally rendered panel, and a clear `aria-label`. Use `<span aria-hidden="true">` around decorative emojis in the text.

...
**Learning:** Buttons that toggle visibility of content (expand/collapse) often miss `aria-expanded` and `aria-controls` attributes, rendering their state changes invisible to screen readers, and inner SVG icons are sometimes read improperly without `aria-hidden`.
**Action:** Always include `aria-expanded` mapped to the open state and `aria-controls` pointing to a unique ID generated by `useId()` for the panel. Ensure structural inner SVG icons are marked with `aria-hidden="true"`.

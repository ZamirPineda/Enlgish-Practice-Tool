...
## 2026-03-12 - [Interactive Flashcard Accessibility]
**Learning:** Custom interactive elements like flashcards need `role="button"`, keyboard handling (Enter/Space), and screen reader instructions. Direct `aria-label`s mask child content. Mixing languages without `lang` attributes breaks screen reader pronunciation.
**Action:** Use `.sr-only` spans for extra instructions to avoid masking content, ensure proper `lang="es"` tags, and add `onKeyDown` handlers for basic navigation.

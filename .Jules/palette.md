...
## 2026-03-12 - [Expandable Content Accessibility with Emojis]

**Learning:** Buttons that toggle content visibility (like "Show Family 👨‍👩‍👧‍👦") can be very verbose and confusing for screen readers if the inner text and emojis are read out alongside the `aria-label`. They also frequently lack `aria-expanded` and `aria-controls`.
**Action:** When creating expandable sections, always provide `aria-expanded` and `aria-controls`. Additionally, if the toggle button contains decorative emojis or text that changes visually but is handled by `aria-label`, wrap the inner content in `<span aria-hidden="true">` to prevent redundant/confusing announcements.

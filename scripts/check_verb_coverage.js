...
const verbsPath = path.join(__dirname, "../src/features/data/verbs.ts");
const grammarVerbsPath = path.join(
  __dirname,
  "../src/features/data/stop_categories/grammar_verbs.ts",
);
...
const irregularBases = new Set(
  irregularBaseMatches.map((m) => m[1].toLowerCase()),
);
...
const missingIrregulars = [...irregularBases].filter(
  (base) => !grammarWords.has(base),
);
...
const irregularLines = grammarVerbsContent
  .split("\n")
...
const missingDefinition = [...grammarIrregulars].filter(
  (word) => !irregularBases.has(word),
);

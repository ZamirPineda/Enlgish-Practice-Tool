...
try {
    ...
    const expectedVerbs = [
        'Accomplish', 'Balance', 'Challenge', 'Debate', 'Estimate',
        'Facilitate', 'Generate', 'Handle', 'Illustrate', 'Lock',
        'Monitor', 'Participate', 'Raise', 'Study', 'Target',
        'Undermine', 'Validate', 'Wander'
    ];

    console.log('\n--- Checking for New Verbs ---');
    const grammarWordsSet = new Set(grammarWords.map(w => w.toLowerCase()));
    const missing = expectedVerbs.filter(v => !grammarWordsSet.has(v.toLowerCase()));
